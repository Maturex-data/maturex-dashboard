  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
      ...
      <Card
        ...
        className="border-border/60 bg-card/60 backdrop-blur-xs shadow-xs hover:border-emerald-500/40 transition-colors cursor-pointer group"
      >
          ...
          <div>
            <div className="flex items-baseline gap-2">
              <span className="text-2xl sm:text-3xl font-bold font-mono tracking-tight text-foreground">
                <AnimatedNumber value={ordersMetric.count} decimals={0} />
              </span>
              <DiffBadge diff={ordersMetric.diffPercent} />
            </div>
      ...
      {/* 2. DOANH THU THUẦN */}
      <Card className="border-border/60 bg-card/60 backdrop-blur-xs shadow-xs hover:border-sky-500/40 transition-colors group">
        <CardContent className="p-4 sm:p-5 flex flex-col justify-between h-full space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-muted-foreground flex items-center gap-1.5">
              <DollarSignIcon className="size-3.5 text-sky-500" />
              Doanh Thu Thuần
              <Badge
                variant="outline"
                className="text-[9px] px-1 py-0 font-mono text-sky-600 border-sky-500/30 bg-sky-500/10"
              >
                calc
              </Badge>
            </span>
          ...
          <div>
            <div className="flex items-baseline gap-2">
              ...
              <DiffBadge diff={netRevenueMetric.diffPercent} />
            </div>
            <div className="mt-1 flex items-center justify-between text-xs text-muted-foreground">
              <span
                title={`Gross sales: $${netRevenueMetric.grossSales.toLocaleString(
                  "en-US",
                  {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  },
                )} | Tax: $${netRevenueMetric.originalTax.toLocaleString(
                  "en-US",
                  {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
      ...
      <Card
        ...
        className="border-border/60 bg-card/60 backdrop-blur-xs shadow-xs hover:border-violet-500/40 transition-colors cursor-pointer group"
      >
        <CardContent className="p-4 sm:p-5 flex flex-col justify-between h-full space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-muted-foreground group-hover:text-foreground transition-colors flex items-center gap-1.5">
          ...
          <div>
            <div className="flex items-baseline gap-2">
              ...
              <DiffBadge diff={adSpendMetric.diffPercent} />
            </div>
            <div className="mt-1 flex items-center justify-between text-xs text-muted-foreground">
              <span>Hệ số MER:</span>
              <span className="font-mono font-medium text-foreground">
      ...
      <Card
        ...
        className="border-border/60 bg-card/60 backdrop-blur-xs shadow-xs hover:border-amber-500/40 transition-colors cursor-pointer group"
      >
        <CardContent className="p-4 sm:p-5 flex flex-col justify-between h-full space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-muted-foreground group-hover:text-foreground transition-colors flex items-center gap-1.5">
              <LayersIcon className="size-3.5 text-amber-500" />
              Lãi Gộp Sau COGS
              <Badge
                variant="outline"
          ...
          <div>
            <div className="flex items-baseline gap-2">
              ...
              <DiffBadge diff={grossProfitMetric.diffPercent} />
            </div>
            <div className="mt-1 flex items-center justify-between text-xs text-muted-foreground">
