            ...
            return (
              <SidebarMenuItem key={item.title}>
                <SidebarMenuButton
                  ...
                  className={`gap-2.5 px-3 py-2 h-9 text-xs font-medium rounded-lg transition-all cursor-pointer ${
                    item.isActive
                  ...
                  render={
                    <Link href={item.url} className="flex w-full items-center">
                      <span className="size-4 shrink-0 text-muted-foreground transition-colors">
                        {item.icon}
                      </span>
                      <span className="truncate ml-2.5">{item.title}</span>
                      {item.badge ? (
                        <span className="ml-auto rounded-md bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 px-1.5 py-0.2 text-[10px] font-mono font-medium">
                          {item.badge}
                        </span>
                      ) : null}
                    </Link>
